...
namespace odfaeg {
    namespace physic {
        ...
        bool BoundingPolyhedron::intersects(math::Ray ray) {
            for (unsigned int i = 0; i < points.size(); i++) {
                   if (ray.getDir().dot2(bissectors3D[i]) != 0) {
                       math::Vec3f pointA = points[i];
                       math::Vec3f pointB = points[(i+1 >= points.size()) ? 0 : points.size() + 1];
                       math::Vec3f pointC;
                       math::Vec3f o, point;
                       if (i + 2 == points.size()) {
                           pointC = points[0];
                       ...
                       && point.y + point.z <= 1) {
                        ...
                        math::Vec3f v2 = points[(i + 1 == points.size()) ? 0 : points.size() + 1];
                        math::Ray r (v1, v2);
        ...
        bool BoundingPolyhedron::intersectsWhere(math::Ray ray, math::Vec3f& i1, math::Vec3f& i2) {
            int nbInt = 0;
            for (unsigned int i = 0; i < points.size(); i++) {
                   if (ray.getDir().dot(bissectors3D[i]) != 0) {
                       math::Vec3f pointA = points[i];
                       math::Vec3f pointB = points[(i+1 >= points.size()) ? 0 : points.size() + 1];
                       math::Vec3f pointC;
                       if (i + 2 == points.size()) {
