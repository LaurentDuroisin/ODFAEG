...
#   pre-link dependencies, we just "link" them so that the sfgl samples can compile
#   out-of-the-box (CMake forwards the dependencies automatically)
macro(sfgl_static_add_libraries target)
...
macro(sfgl_list_contains var value)
    set(${var})
    foreach(value2 ${ARGN})
...
# ex: sfgl_parse_arguments(THIS "SOURCES;DEPENDS" "FLAG" FLAG SOURCES s1 s2 s3 DEPENDS d1 d2)
# will define the following variables:
# - THIS_SOURCES (s1 s2 s3)
# - THIS_DEPENDS (d1 d2)
# - THIS_FLAG TRUE
macro(sfgl_parse_arguments prefix arg_names option_names)
    ...
    foreach(arg ${ARGN})
        sfgl_list_contains(is_arg_name ${arg} ${arg_names})
        if(is_arg_name)
            set(${prefix}_${current_arg_name} ${current_arg_list})
            set(current_arg_name ${arg})
            set(current_arg_list)
        else()
            sfgl_list_contains(is_option ${arg} ${option_names})
            if(is_option)
...
# add a new target which is a sfgl library
# ex: sfgl_add_library(sfgl-graphics
#                      SOURCES sprite.cpp image.cpp ...
#                      DEPENDS sfgl-window sfgl-system
#                      EXTERNAL_LIBS opengl freetype ...)
macro(sfgl_add_library target)

    # parse the arguments
    sfgl_parse_arguments(THIS "SOURCES;DEPENDS;EXTERNAL_LIBS" "" ${ARGN})
    ...
    set_target_properties(${target} PROPERTIES FOLDER "sfgl")

    # for gcc >= 4.0 on Windows, apply the sfgl_USE_STATIC_STD_LIBS option if it is enabled
    if(WINDOWS AND COMPILER_GCC AND sfgl_USE_STATIC_STD_LIBS)
    ...
    # link the target to its sfgl dependencies
    if(THIS_DEPENDS)
    ...
    if(MACOSX AND BUILD_SHARED_LIBS)
        if(sfgl_BUILD_FRAMEWORKS)
            ...
            set_target_properties(${target} PROPERTIES 
                                  ...
                                  MACOSX_FRAMEWORK_IDENTIFIER org.sfgl-dev.${target}
    ...
    if(THIS_EXTERNAL_LIBS)
        if(BUILD_SHARED_LIBS)
            ...
            sfgl_static_add_libraries(${target} ${THIS_EXTERNAL_LIBS})
        endif()
    endif()
...
# add a new target which is a sfgl example
# ex: sfgl_add_example(ftp
#                      SOURCES ftp.cpp ...
#                      DEPENDS sfgl-network sfgl-system)
macro(sfgl_add_example target)

    # parse the arguments
    sfgl_parse_arguments(THIS "SOURCES;DEPENDS" "GUI_APP" ${ARGN})
    ...
    if(THIS_GUI_APP AND WINDOWS)
        ...
        target_link_libraries(${target} sfgl-main)
    else()
